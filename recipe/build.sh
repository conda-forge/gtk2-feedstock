...
if [[ "${target_platform}" != win-* ]]; then
  ...
  powershell -Command "(Get-Content configure) -replace '\$PKG_CONFIG --uninstalled', 'false || $PKG_CONFIG --uninstalled' | Set-Content configure.new; Move-Item configure.new configure -Force; $uninstalledLines = (Get-Content configure | Select-String -Pattern '\$PKG_CONFIG --uninstalled'); if ($uninstalledLines) { Write-Host 'Lines containing --uninstalled:'; $uninstalledLines; exit 1 } else { exit 0 }"
